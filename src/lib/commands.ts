...
export const commands: Record<CommandKey, Command> = {
  ...
  projects: {
    ...
    execute: () => {
      const content = projectsData.projects
        .map((p, i) =>
          [
            `${i + 1}. ${p.title}`,
            `   ${p.description}`,
  ...
  skills: {
    ...
    execute: () => {
      ...
      const content = skillsData.skills
        .map((s) =>
          [s.name, `${bar(s.level)} ${s.level}%`, s.keywords, ""].join("\n")
        )
        .join("");
  ...
  theme: {
    ...
    execute: ({ theme, setTheme }, args?: string) => {
      ...
      if (themes[newTheme as ThemeKey]) {
        ...
        return {
          type: "success",
          content: `✓ Theme changed to: ${
            themes[newTheme as ThemeKey].name
          }\n\nLooking good! 😎`,
        };
      }
      return {
        type: "error",
        content:
          "Theme not found. Available: matrix, dracula, monokai, cyberpunk, hacker",
      };
    },
  },
  themes: {
